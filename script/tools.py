...
def crack_pwd(pwd, key):
    ...
    cipher_text = base64.b64encode(
        cipher.encrypt(pwd.encode(encoding="utf-8"))
    )  # 对传递进来的用户名或密码字符串加密
...
def list_to_tree(node_list, root_id=None, *, order="", exclude=None):
    ...
    # 排序
    if order:
        node_list.sort(key=lambda k: k.get(order))
    # node
    node_dict = {
        node["id"]: (
            node if node.get("parent_id") else node.update({"parent_id": -1}) or node
        )
    ...
    for node in node_list:
        if node["id"] == exclude:
            continue
        node_dict.setdefault(node["parent_id"], {}).setdefault("children", []).append(
            node
        )
    if root_id:
...
def dfs_tree_to_list(nodes):
    ...
    for node in nodes:
        ...
        if children != []:
            ids = ids + dfs_tree_to_list(children)
    return ids
    ...
    if length < default_length:
        ...
        while length - offset > 0:
            if length - offset > default_length:
                res.append(
                    decrypter.decrypt(
                        ciphertext[offset : offset + default_length], b" "
                    )
                )
            else:
...
if __name__ == "__main__":
    ...
    encode_str = ""
    decode_str = aes_cbc_decode(encode_str, aes_key, aes_iv)
    print(decode_str)
